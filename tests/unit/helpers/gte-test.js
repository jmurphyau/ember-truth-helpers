...
module('helper:gte', function(hooks) {
  ...
  test('boolean values', async function(assert) {
    ...
    assert.equal(this.element.textContent, '[true] [true] [false] [true]', 'value should be "[false] [true] [false] [true]"');
  });

  test('integer values', async function(assert) {
    await render(hbs`[{{gte 1 1}}] [{{gte 1 0}}] [{{gte 0 1}}] [{{gte 0 0}}]`);

    assert.equal(this.element.textContent, '[true] [true] [false] [true]', 'value should be "[true] [true] [false] [true]"');
  });
