...
test('boolean values', function() {
  ...
  equal(view.$().text(), '[true] [false] [false] [false]', 'value should be "[true] [false] [false] [false]"');
});

test('integer values', function() {
  registerHelper('and',andHelper);
  ...
  equal(view.$().text(), '[1] [0] [0] [0]', 'value should be "[1] [0] [0] [0]"');
});
